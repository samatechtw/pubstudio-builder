...
export const joinUp: Command = (state, dispatch) => {
  let sel = state.selection,
    ...
    point
  if (nodeSel) {
    if ((sel as NodeSelection).node.isTextblock || !canJoin(state.doc, sel.from))
      return false
    point = sel.from
  } else {
...
export function splitBlockAs(
  ...
  return (state, dispatch) => {
    ...
    if (dispatch) {
      ...
      // Originally this was `dispatch(tr.scrollIntoView())`. `scrollIntoView()` was removed
      // owing to https://github.com/samatechtw/pubstudio-builder/issues/173.
      dispatch(tr)
    }
    return true
  }
...
function markApplies(doc: Node, ranges: readonly SelectionRange[], type: MarkType) {
  for (let i = 0; i < ranges.length; i++) {
    ...
    let can =
      $from.depth == 0 ? doc.inlineContent && doc.type.allowsMarkType(type) : false
