/**
 * This file is a copy of "src/getHighlightDecorations.ts" from "prosemirror-highlightjs" library.
 * @see https://github.com/b-kelly/prosemirror-highlightjs/blob/master/src/getHighlightDecorations.ts
 */
...
interface GetHighlightDecorationsOptions {
  /**
   * A method that is called before the render process begins where any non-null return value cancels
   * the render; useful for decoration caching on untouched nodes
   * @param block The node that is about to render
   * @param pos The position in the document of the node
   * @returns An array of the decorations that should be used instead of rendering; cancels the render
   * if a non-null value is returned
   */
  preRenderer?: (block: ProseMirrorNode, pos: number) => Decoration[] | null

  /**
   * A method that is called after the render process ends with the result of the node render passed;
   * useful for decoration caching
  ...
  /**
   * A method that is called when a block is autohighlighted with the detected language passed;
   * useful for caching the detected language for future use
   * @param block The node that was renderer
   * @param pos The position of the node in the document
 ...
 * @param languageExtractor A method that is passed a prosemirror node and returns the language
 * string to use when highlighting that node
