/**
 * This file is a copy of "src/plugin.ts" from "prosemirror-highlightjs" library, with
 * the `view` spec object commented out.
 * @see https://github.com/b-kelly/prosemirror-highlightjs/blob/master/src/plugin.ts
 */
...
import { getHighlightDecorations } from './get-highlight-decorations'
...
/**
 * Creates a plugin that highlights the contents of all nodes (via Decorations) with a
 * type passed in blockTypes
 ...
 * @param languageExtractor A method that is passed a prosemirror node and returns the
 * language string to use when highlighting that node; defaults to using `node.attrs.params`
 * @param languageSetter A method that is called after language autodetection of a node in
 * order to save a autohighlight value for future use
 */
export function highlightPlugin(
  ...
  return new Plugin<HighlightPluginState>({
    ...
    // `prosemirror-highlightjs` use this function to update node's language in a editor,
    // which leads to a "Maximum call stack size exceeded" error. Since we only need to
    // format JS code at the moment, we can just skip the following part for now.
