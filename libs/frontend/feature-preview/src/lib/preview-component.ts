      ...
      return () => {
        ...
        if (component.value.tag === Tag.A) {
          ...
          if (/^(https?:\/\/|www\.)/.test(hrefProp)) {
            ...
            // the href generated by the required prop `to`.
            // If `to` and `href` are both present, <router-link> will use the value of `href` prop as
            // the href of the rendered <a>, but upon click, users will actually be navigated to the
            // href generate by `to`.
            // To avoid confusion, it's better to use `to` alone here.

            const { search, hash } = window.location
            const copiedRenderProps = {
              ...renderProps,
              to: `${hrefProp}${search}${hash}`,
            }
