...
MAILSENDER_API_KEY      | Optional secret key for sending mail via MailerSend
...
```bash
docker build -t site-api --build-arg="PLATFORM_FOLDER=" -f backend/site-api/Dockerfile_dev --target=dev  .
