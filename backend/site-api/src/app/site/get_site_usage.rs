...
use lib_shared_site_api::{cache::cache::SiteUsageData, error::api_error::ApiError};
...
) -> Result<Json<SiteUsageData>, ApiError> {
    verify_site_owner(&context, &user, &site_id).await?;

    let data = context.cache.get_usage(&site_id).await?;
